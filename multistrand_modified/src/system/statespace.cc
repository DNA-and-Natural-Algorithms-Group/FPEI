...
void Builder::addState(ExportData& data, const double arrType) {
	...
	if (lastState.complex_count > 0) {
		...
		trans.state2 = data;
		trans.type = arrType;
		ExportTransition transstr =std::move(trans) ;
		protoTransitions.insert(transstr);
		//totalcounter +=1 ;
		//cout << "::::::::" << totalcounter <<"\n"; 
		
		if (protoTransitionsCount.find(transstr) == protoTransitionsCount.end()) {
			protoTransitionsCount[transstr] =  1 	; 
...
void Builder::writeToFile(void) {
	...
	//transitionsCount
	myfile.open(filename("prototransitionscount"));

	for (auto element : protoTransitionsCount) {
