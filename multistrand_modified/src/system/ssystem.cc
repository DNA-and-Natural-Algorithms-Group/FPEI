...
void SimulationSystem::StartSimulation_Trajectory(void) {
	...
	while (simulation_count_remaining > 0) {
		...
		ofstream myfile_fptormfptssa;
		ofstream myfile_fptormfptrvssa;
		
		ofstream myfile_meanssa;
		ofstream myfile_meanrvssa;
		ofstream myfile_variancessa;
		ofstream myfile_variancervssa;
		ofstream myfile_trajectorylength_n;
		
		
		SimTimer t= SimulationLoop_Trajectory();
		finalizeRun();
		st += t.stime ; 
		st2 += t.stime2; 
		//in simtimer.cc stime is using expected holding time as in RVSSA and stime2 is using random holding time as in SSA
		st_samples.push_back (t.stime);
		st2_samples.push_back (t.stime2);
		cout << "RVSSA: " <<  "		Mean: "<< st/st_samples.size() <<"\n";
		cout << "SSA: " << "		Mean: " << st2/st2_samples.size()  << "\n"; 
		cout << "--------------------------------------------------------" <<"\n"; 
	}
}
...
SimTimer SimulationSystem::SimulationLoop_Trajectory() {
