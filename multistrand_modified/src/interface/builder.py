...
'''  This file is modified by NZ to incorporate FPEI '''
...
class Builder(object):
    ...
    def __init__(self, inputFunction, arguments):
        ...
        self.protoTransitions_FPEI = dict() #used for FTEI
        self.protoTransitionsCount =  dict( ) #count the number of times each edge appears
    ...
    def reset(self):
        ...
        #added by nz
        self.protoTransitions_FPEI.clear( )
        self.protoTransitionsCount.clear()
    ...
    def mergeProtoTransitionCount(self, this, that):
    ...
    def mergeBuilder(self, other):
        ...
        self.mergeProtoTransitionCount(self.protoTransitionsCount, other.protoTransitionsCount)
        ...
        try :
            print "starting to merge neighbors_FPEI"
            self.mergeSet(self.neighbors_FPEI , other.neighbors_FPEI)
            self.mergeSet(self.protoTransitions_FPEI , other.protoTransitions_FPEI)
            print "fiished mergining neighbors_FPEI"
        except:
            print "no neighbors_FPEI"
    ...
    def genUntilConvergenceWithInitialState_doublesim(self, precision, initialStates, printMeanTime=False, fatten_regardless = False):
        ...
        breakloop = False
        if len (self.protoSpace) >= precision or fatten_regardless == True:
            self.fattenStateSpace()
            breakloop = True
        if self.verbosity:
            print "Size     = %i " % len(self.protoSpace)
    ...
    def fattenStateSpace(self, start  =   None , end = None ):
        ...
        def inspectionSim(inputs):
            ...
            #added by nz nz, these lines should be here or the energy of the states will be different
            # This effects the _FPEI  rates!
        ...
        for key, value in self.protoSpacebackup.iteritems():
            ...
            myB.uniqueID_number = self.uniqueID_number
            myB.genAndSavePathsFile(inspecting=True)


            self.mergeSet(self.tempstatespace, myB.protoSpace)
            self.mergeSet(self.temptransitions, myB.protoTransitions)
    ...
    def genAndSavePathsFile(self, ignoreInitialState=False, supplyInitialState=None, inspecting=False):
        ...
        def runPaths(optionsF, optionsArgs, space, transitions, initStates, finalStates, sequences):
            ...
            totalcountbuilder= 0
            """ load the transitionscount """
            with open(self.the_dir + str(myOptions.interface.current_seed) + "/prototransitionscount.txt", "r" ) as myFile:
                ...
                while go_on:
                    ...
                    line0= myLines[index]
                    line1 = myLines[index+1];
                    line2 = myLines[index + 2];
                    line3 = myLines[index + 3];


                    index = index + 6 # note the whitespace
                    ...
                    if not transitionPair in transitionsCount:
                        transitionsCount[transitionPair] = int(line0)
                        totalcountbuilder  += int(line0 )
                    else :
                        transitionsCount[transitionPair] += int(line0)
                        totalcountbuilder  += int(line0 )
            ...
            os.remove(self.the_dir + str(myOptions.interface.current_seed) + "/prototransitionscount.txt")
        ...
        #NZ: the  unique ids of the states are strings which uses a lot of memory. Converting the unique IDs to integers to reduce memory usage
        for state in space:
            if state not in self.uniqueID_number:
                self.uniqueID_number[state] =  len(self.uniqueID_number)

        space2 = dict()
        for state in space:
            space2 [self.uniqueID_number[state]]  = space[state]
        del space
        self.mergeSet(self.protoSpace, space2)
        del space2
        ...
        for transit in transitions :
            transitions2[ self.uniqueID_number[transit[0]],  self.uniqueID_number[transit[1]]  ]  = transitions [ transit]
        del transitions
        self.mergeSet(self.protoTransitions, transitions2)


        transitionsCount2 = dict()
        for transit in transitionsCount :
            transitionsCount2[ self.uniqueID_number[transit[0]],  self.uniqueID_number[transit[1]]  ]  = transitionsCount [ transit]
        self.mergeProtoTransitionCount(self.protoTransitionsCount, transitionsCount2)

        finalStates2 = dict()
        for state in finalStates:
            finalStates2 [self.uniqueID_number[state]]  = finalStates[state]
        del finalStates
        self.mergeSet(self.protoFinalStates, finalStates2)
        del finalStates2
        ...
        for state in sequences:
            sequences2 [self.uniqueID_number[state]]  = sequences[state]

        del sequences
        self.mergeSet(self.protoSequences, sequences2)
        del sequences2


        initStates2 = dict()
        for key, val in initStates.iteritems() :
            initStates2[self.uniqueID_number[key]   ] = val
        del initStates
        ...
        if not ignoreInitialState:
            ...
            for key, val in initStates2.iteritems():
...
class BuilderRate(object):
    ...
    def genNeighborsTransitive_FPEI(self, transitions, statespace):
    ...
    def processStates(self):
        ...
        self.build.neighbors_FPEI = self.genNeighborsTransitive_FPEI(self.build.protoTransitions_FPEI, self.build.protoSpace)
    ...
    def get_rate(self, state1, state2):
        ...
        transitionlist = self.build.protoTransitions_FPEI[(state1, state2)]
    ...
    def metropolis_rate(self, state1, state2, transitionlist, greater= None , dgsum  = None, dg2sum= None , n = None ):
        ...
        if greater == None:
            ...
            if transitionlist[0] == transitiontype.unimolecular:
                if dG1 > dG2 :  # state2 is more stable (negative), dG1 - dG2 is positive
                    ...
                    rate2 = self.build.options.unimolecular_scaling * np.exp(-DeltaG2  / RT)
                else:  # state2 is less or equally stable (negative), dG1 - dG2 is negative
                    rate1 = self.build.options.unimolecular_scaling * np.exp(-DeltaG  / RT)
                ...
                if transitionlist[0] == transitiontype.bimolecularIn:
                    outR = self.build.options.bimolecular_scaling * np.e ** (- DeltaG2  / RT)
                    return collisionRate, outR

                elif transitionlist[0] == transitiontype.bimolecularOut:
                    outR = self.build.options.bimolecular_scaling * np.e ** (-DeltaG  / RT)
            ...
            if transitionlist[0] == transitiontype.unimolecular:
                if greater < 0.0 :  # state2 is more stable (negative), dG1 - dG2 is positive
                    rate1 = self.build.options.unimolecular_scaling *  n
                    #rate2 = self.build.options.unimolecular_scaling * dg2sum
                else:  # state2 is less or equally stable (negative), dG1 - dG2 is negative
                    rate1 = self.build.options.unimolecular_scaling * dgsum
                    #rate2 = self.build.options.unimolecular_scaling * n
                return rate1,  "the value of this should not matter"
            else:  # bimolecular rate
                collisionRate = self.build.options.join_concentration * self.build.options.bimolecular_scaling * n
                if transitionlist[0] == transitiontype.bimolecularIn:
                    #outR = self.build.options.bimolecular_scaling * dg2sum
                    return collisionRate,  "the value of this should not matter"
                elif transitionlist[0] == transitiontype.bimolecularOut:
                    outR = self.build.options.bimolecular_scaling * dgsum
                    return outR,  "the value of this should not matter"
                else:
                    raise ValueError('The transition code is unexpected ')

    def arrhenius_rate(self, state1, state2, transitionlist, greater= None , dgsum  = None, dg2sum= None , n = None ):
        ...
        if greater== None:
            ...
            if transitionlist[0] == transitiontype.unimolecular:
                if greater> 0.0:
                    rate1 = np.e ** (lnA - E / RT)   * ( dgsum )
                    #rate2 = np.e ** (lnA - E / RT) * n
                else:
                    rate1 = np.e ** (lnA - E / RT)* n
                    #rate2 = np.e ** (lnA - E / RT)   * (  dg2sum  )
            elif transitionlist[0] == transitiontype.bimolecularIn:
                rate1 = bimolecular_scaling * concentration * (np.e ** (lnA - E / RT)) * n
                #rate2 = bimolecular_scaling * (np.e ** (lnA - E / RT))  * (  dg2sum )
            elif transitionlist[0] == transitiontype.bimolecularOut:
                rate1 = bimolecular_scaling * (np.e ** (lnA - E/ RT)) * ( dgsum  )   * n
                #rate2 = bimolecular_scaling * concentration * (np.e ** (lnA - E / RT))
            else :
                raise ValueError('Exception in transition rate calculations.')
